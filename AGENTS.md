...
- Prefer inline component attributes (`name:Type(x:..., y:...)`) over legacy `@id(...)`/`name:Type@state` for new examples and docs; keep `@layout(...)` and action states (for example `@db.read(...)`) for global layout and action timelines.
