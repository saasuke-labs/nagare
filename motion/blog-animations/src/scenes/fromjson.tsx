...
const JSONData = {
  nodes: [
    {
      id: "flowchart-B-8",
      label: "Browser", // url TBD:  https://example.com
      classList: ["node", "default", "Browser"],
      bbox: {
        x: -116.46875,
        y: -17,
        width: 232.9375,
        height: 150, // changed from 34 to 150
      },
    },
    {
      id: "flowchart-VM-9",
      label: "VM",
      classList: ["node", "default", "VM"],
      bbox: {
        x: 250, // Changed to 250 due to the width of the Browser
        y: -17,
        width: 230, // Changed from 35 to 230
        height: 150, // Changed from 34 to 150
      },
    },
  ],
  edges: [
    {
      from: "B",
      to: "VM",
      fromId: "flowchart-B-8",
      toId: "flowchart-VM-9",
      label: "request",
    },
  ],
  classDefs: {
    Browser: "fill:#fff,stroke:#222;",
    VM: "fill:#333,stroke:#ccc;",
  },
}

export default makeScene2D(function* (view) {
  const references: { [key: string]: Reference<Browser> | Reference<VM> } = {}

  const classes: { [key: string]: { [key: string]: string } } = {}

  for (const classDef of Object.entries(JSONData.classDefs)) {
    const [className, styleString] = classDef

    const styleEntries = styleString.split(",").reduce((obj, entry) => {
      const [key, value] = entry.split(":")

      return { ...obj, [key]: value.trim() }
    }, {})

    classes[className] = styleEntries
  }

  JSONData.nodes.forEach((node) => {
    if (node.classList.includes("Browser")) {
      references[node.id] = createRef<Browser>()
      ...
      view.add(
        <Browser
          ...
          title={node.label}
          titleBarColor={classes["Browser"]?.fill || "#222"}
          contentColor={classes["Browser"]?.fill || "#fff"}
          opacity={1}
        />
      )
    } else if (node.classList.includes("VM")) {
      references[node.id] = createRef<VM>()
